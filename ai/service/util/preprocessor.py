...
class FeatureEngineering(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # # 무한대 값 확인 (열 별로)
        # infinity_counts = np.isinf(X.select_dtypes(include=[np.number])).sum()

        # print("각 피처별 무한대 값 개수:")
...
class FeatureImportanceFilter(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        # 피처 중요도 저장 및 중요도가 임계값 이상인 피처 선택
        ...
        # 임계값보다 큰 중요도를 가진 피처만 선택
        self.selected_features_ = feature_importances_df[
            feature_importances_df["Importance"] > self.threshold
        ]["Feature"].tolist()
    ...
    def transform(self, X):
        ...
        # print('1', X[self.selected_features_].shape)
...
class CorrelationFilter(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # print('2',X[self.selected_features_].shape)
        # 선택된 피처만 반환
        return X[self.selected_features_]
...
class FeatureSelection(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if missing_features:
            ...
            # 필요에 따라 누락된 피처를 제외하거나 예외를 발생시킬 수 있습니다.
            # raise KeyError(f"{missing_features} not in columns")
